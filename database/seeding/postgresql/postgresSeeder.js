...
const generateInventory = () => {
  ...
  const generateInventoryItem = (size) => {
    const product_id = random.number({ min: 1, max: totalUniqueProducts});
    const variant_id = random.number({ min: 1, max: totalProductVariants});

    //simulate 20% of sizes are 0
    const stock = random.number({ min: 0, max: 4});

    return `${product_id},${variant_id},${size},${stock}\n`;
  }
  ...
  const write = () => {
    ...
    while (variant_id <= totalProductVariants && canWrite) {
      ...
      while (curSize <= largestSize && canWrite) {
        ...
        let row = `${variant_id},${curSize},${stock}\n`
...
// stream.write(inventoryHeader, 'utf-8');
// writeCSV(stream, 'utf-8', () => {
//   stream.end()
// }, generateInventory)
