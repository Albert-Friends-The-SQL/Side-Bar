import http from 'k6/http';
import { sleep } from 'k6';
import faker from 'https://cdnjs.cloudflare.com/ajax/libs/Faker/3.1.0/faker.min.js';

export let options = {
  vus: 10,
  duration: '30s'
};



export default function () {
  const startingVariantId = 25000001;

  const variant_id = __ITER + startingVariantId;
  const product_id = faker.random.number({ min: 1, max: 10000000});
  const variant_name = faker.lorem.words();
  const ratings = faker.random.number({ min: 1, max: 1000});
  const category_1 = faker.lorem.word();
  const category_2 = faker.lorem.word();
  //simulate 25% of products are unisex
  const unisex = faker.random.number({ min: 1, max: 4}) === 1
  const reg_price = faker.random.number({ min: 100, max: 200});
  const color_1 = faker.commerce.color();
  const color_2 = faker.commerce.color();
  const color_3 = faker.commerce.color();

  const url = 'http://localhost:3000/api/variants/';
  let headers = { 'Content-Type': 'application/json' };

  let data = {
    variant_id,
    product_id,
    variant_name,
    ratings,
    category_1,
    category_2,
    unisex,
    reg_price,
    color_1,
    color_2,
    color_3,
  }
  console.log(variant_id);
  let res = http.post(url, JSON.stringify(data), { headers });
  headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
  res = http.post(url, data, { headers });
}
