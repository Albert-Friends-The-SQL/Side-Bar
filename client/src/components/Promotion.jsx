...
import Shipping from './Modals/Shipping/Content.jsx';
import Payment from './Modals/Payment/Content.jsx';
  ...
  return (
    <div>
      <div style={{display: 'flex', 'margin-top': '12px'}}>
        ...
        <div className="promotion" onClick={() => setIsOpenPayment(true)}>
        PAY OVER TIME IN INTEREST-FREE INSTALLMENTS WITH AFFIRM, KLARNA OR AFTERPAY
        </div>
        <Payment open={isOpenPayment} onClose={() => setIsOpenPayment(false)}/>
      </div>
      <div style={{display: 'flex'}}>
        ...
        <div className="promotion" onClick={() => {setIsopenShipping(true)}}>
          FREE SHIPPING + FREE RETURNS & EXCHANGES
        </div>
        <Shipping open={isOpenShipping} onClose={() =>setIsopenShipping(false)}/>
      </div>
